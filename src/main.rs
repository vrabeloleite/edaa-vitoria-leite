...
// Primeiro exercício que fiz, demorei pra lembrar que pop() inverte naturalmente
...
// Tive que refazer esse aqui porque tava dando erro de borrow, usei if let pra resolver
// Custo de Tempo: O(n) | Custo de Espaço: O(n)
...
// Esse aqui a ideia da pilha auxiliar é muito boa, não tinha pensado nisso
pub struct StackMin {
...
// Demorei pra entender que overwrite é descartar o mais antigo, não recusar o novo
...
// Sei que depois vamos aprender Heap pra fazer isso O(log n), mas por enquanto é O(n)
...
// Esse foi o mais difícil! O Deque guarda índices, não valores
// Tive que assistir uns vídeos no YouTube pra entender direito
...
// Primeira tentativa deu loop infinito, esqueci de tratar quantum maior que o restante
...
mod tests {
    ...
    #[test]
    fn test_round_robin() {
        // Teste básico pra garantir que não dá loop infinito
        let processos = VecDeque::from(vec![
            Processo { id: 1, tempo_restante: 5 },
            Processo { id: 2, tempo_restante: 3 },
        ]);
        // Só chamando pra ver se executa sem travar
